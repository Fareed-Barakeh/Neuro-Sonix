...
# Function to convert a word to a MIDI sequence with slower note lengths
    ...
    for char in word:
        try:
            ...
            if char.isalpha() and char in note_to_midi:
                ...
                # Generate a random note length (in milliseconds) between 100 and 200 ms
                note_length = random.randint(100, 200)
                # Add a note-on message for each MIDI note with the slower note length
...
# Iterate through the words and convert them to MIDI sequences with slightly slower note lengths
for i, word in enumerate(words):
    ...
    if midi_sequence:
        ...
        if i != len(words) - 1:
            space_length = random.randint(50, 250)  # Random space length between 300 and 500 ms
            track.append(Message('note_off', note=0, velocity=0, time=space_length))

desktop_path = os.path.expanduser("~/Desktop/")

# Save the MIDI file on the desktop
midi_file_path = os.path.join(desktop_path, "8.mid")
